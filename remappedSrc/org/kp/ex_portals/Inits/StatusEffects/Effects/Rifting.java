...
public final class Rifting extends StatusEffect {
    ...
    public void applyUpdateEffect(LivingEntity entity, int amplifier) {
        if(RiftItems.RiftSeekerChestplate == entity.getEquippedStack(EquipmentSlot.CHEST).getItem()) {
            entity.damageArmor(new DamageSource(entity.method_48926().getRegistryManager().get(RegistryKeys.DAMAGE_TYPE).entryOf(StatusEffectRegistry.Riften)), 10);
        }
        else
        {
            entity.damage(new DamageSource(entity.method_48926().getRegistryManager().get(RegistryKeys.DAMAGE_TYPE).entryOf(StatusEffectRegistry.Riften)), 0.2f * amplifier);
        }
